	    ...
	    if(!message.guild.me.hasPermission("ADMINISTRATOR") && !message.guild.me.hasPermission("BAN_MEMBERS")){
			...
			member.ban().then((member) => {
				...
				if (!message.member.hasPermission(['BAN_MEMBERS', 'ADMINISTRATOR'])) {
					const error = new Discord.MessageEmbed()
			        	.setColor("#ff0000")
			            .addField(":x: Failed !", `You cannot ban members ${message.author} !`);
	        		return message.channel.send({embed: error}).then(msg => msg.delete({timeout: 10000}));
				} else if (member.user.id == message.author.id){
					const error = new Discord.MessageEmbed()
			        	.setColor("#ff0000")
			            .addField(":x: Failed !", `You cannot ban yourself ${message.author} !`);
	        		return message.channel.send({embed: error}).then(msg => msg.delete({timeout: 10000}));
				} else if (member.hasPermission(['KICK_MEMBERS', 'BAN_MEMBERS', 'ADMINISTRATOR'])) {
					const error = new Discord.MessageEmbed()
			        	.setColor("#ff0000")
			            .addField(":x: Failed !", `You cannot ban this member ${message.author} !`);
	        		return message.channel.send({embed: error}).then(msg => msg.delete({timeout: 10000}));
				}
			});
		}
